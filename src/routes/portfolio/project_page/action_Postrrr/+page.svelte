<script>
    ...
    beforeNavigate(({to, from}) => {
        pageLoaded = false;
        if ( from?.url.pathname == "/portfolio/project_page/action_Postrrr" && to?.url.pathname == undefined ) {
...
<main>
    ...
    <div class="default_container endless">
        ...
        <div class="content_container" in:scale={{ delay: 0, duration: 2000, easing: elasticOut, start: 1.03, opacity: 1 }}>
            ...
            <div class="project_name"> Postrrr </div>
...
<style>
    ...
    *::selection{
    ...
    .project_name::before{
        ...
        position: absolute;
        inset: 0;
        background: radial-gradient(var(--background_color_lightYellow) 65%, hsl(0, 0%, 53%) 200%);
        border: max(0.5rem, 0.5vw) hsl(0, 0%, 55%) dashed;
        z-index: -1;
